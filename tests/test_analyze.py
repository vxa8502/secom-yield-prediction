...
class TestReportGeneration:
    ...
    def test_report_header_format(self):
        ...
        assert '0.070' in result or '0.07' in result
        assert '0.7134' in result
    ...
    def test_report_misclassification_with_data(self):
        ...
        assert 'Misclassification Analysis' in result
        assert '2 false negatives' in result
    ...
    def test_report_misclassification_empty(self):
        ...
        assert 'No false negatives' in result
    ...
    def test_report_missingness_with_data(self):
        ...
        assert 'Performance by Missingness' in result

    def test_report_missingness_empty(self):
        """Test missingness section without data."""
        missingness_df = pd.DataFrame()
    ...
    def test_report_recommendations(self):
        """Test recommendations section contains key advice."""
        result = _report_recommendations()

        assert 'Platt scaling' in result or 'calibration' in result.lower()
        assert 'missingness' in result.lower()
...
class TestGenerateAnalysisReport:
    ...
    def test_creates_markdown_file(self):
        ...
        fn_df = pd.DataFrame({'rank': [1], 'test_index': [10], 'predicted_proba': [0.1]})
        residual_analysis = {
            'overall': {'mean': 0.1, 'std': 0.3, 'skewness': 0.5, 'kurtosis': 1.2},
            'passes': {'mean': -0.1, 'std': 0.2, 'n_samples': 100},
            'failures': {'mean': 0.8, 'std': 0.1, 'n_samples': 10},
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch('pipelines.analyze.REPORTS_DIR', Path(tmpdir)):
                ...
                assert '# Interpretability Analysis Report' in content
    ...
    def test_report_contains_all_sections(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch('pipelines.analyze.REPORTS_DIR', Path(tmpdir)):
                ...
                assert '## Residual Distribution Analysis' in content
                assert '## Performance by Missingness' in content
                assert '## False Negative Cluster Analysis' in content
